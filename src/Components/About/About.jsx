...
const SKILL_CATEGORIES = [
  {
    ...
    icon:  "cube",
    color: T.orange,
    skills: ["Three.js", "WebGL", "GLSL Shaders", "React Three Fiber", "Framer Motion", "Canvas API", "SVG Animation"],
  },
  {
    title: "Frontend",
    icon:  "layers",
    color: T.blue,
    skills: ["React.js", "Next.js 15", "TypeScript", "JavaScript ES6+", "Tailwind CSS", "HTML5 / CSS3", "Vite"],
  },
  {
    title: "Backend & Tools",
    icon:  "terminal",
    color: T.purple,
    skills: ["Node.js", "Express.js", "REST APIs", "PostgreSQL", "MongoDB", "Supabase", "Firebase", "Docker", "Git / GitHub"],
  },
];

const ACHIEVEMENTS = [
  { count: "25+", label: "Projects Shipped",  detail: "From concept to live production",    icon: "zap"    },
...
const TIMELINE = [
  {
    ...
    detail: "Fell into React and never looked back — built 3 projects in the first month.",
    icon:   "⚡",
    color:  T.orange,
  },
  {
    ...
    detail: "Shipped a full e-commerce site from scratch — payments, auth, the lot.",
    icon:   "🚀",
    color:  T.gold,
  },
  {
    ...
    detail: "3D on the web became an obsession. GLSL shaders kept me up til 3 AM.",
    icon:   "◈",
    color:  T.orangeG,
  },
  {
    year:   "2024",
    title:  "First major WebGL project",
    detail: "ConotexTech launched — interactive 3D configurator for textile manufacturing.",
    icon:   "✦",
    color:  T.orange,
  },
  {
    ...
    detail: "15+ projects shipped across 5+ countries. Featured in WebGL showcase.",
    icon:   "🌍",
    color:  T.green,
  },
  {
    ...
    detail: "Available for remote opportunities worldwide. Let's create something extraordinary.",
    icon:   "⚡",
    color:  T.orange,
    active: true,
  },
...
const LIVE_PROJECTS = [
  ...
  },
  {
    ...
    impact:  "3× session duration vs avg",
    year:    "2024",
    status:  "live",
  },
  {
...
const ICONS = {
  ...
  layers: (sz = 11, c = T.orange) => (
    <svg width={sz} height={sz} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <polygon points="12 2 2 7 12 12 22 7 12 2"/><polyline points="2 17 12 22 22 17"/><polyline points="2 12 12 17 22 12"/>
    </svg>
  ),
  terminal: (sz = 11, c = T.orange) => (
    <svg width={sz} height={sz} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
      <polyline points="4 17 10 11 4 5"/><line x1="12" y1="19" x2="20" y2="19"/>
    </svg>
  ),
  zap: (sz = 11, c = T.orange) => (
    <svg width={sz} height={sz} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  externalLink: (sz = 10, c = "currentColor") => (
  ...
  check: (sz = 11, c = T.green) => (
    <svg width={sz} height={sz} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      <polyline points="20 6 9 17 4 12"/>
    </svg>
  ),
  copy: (sz = 11, c = "currentColor") => (
    <svg width={sz} height={sz} viewBox="0 0 24 24" fill="none" stroke={c} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
function AboutThreeScene({ isMobile }) {
  ...
  useEffect(() => {
    ...
    const addShape = (geo, x, y, z, scale, opacity, rotSpeed) => {
      ...
      scene.add(m);
      shapes.push({
        mesh: m, rotSpeed, phase: Math.random() * Math.PI * 2,
        floatAmp: 0.06 + Math.random() * 0.1, floatSpeed: 0.3 + Math.random() * 0.4,
    ...
    const knotMat = new THREE.MeshPhysicalMaterial({
      color: 0xe8622a, metalness: 1.0, roughness: 0.05,
      transparent: true, opacity: 0.0, clearcoat: 1.0, clearcoatRoughness: 0.06,
    });
    ...
    for (let i = 0; i < ptCount; i++) {
      ...
      const b = Math.random();
      ptCol[i * 3] = 0.91 + b * 0.09; ptCol[i * 3 + 1] = 0.38 + b * 0.19; ptCol[i * 3 + 2] = 0.16 + b * 0.05;
    }
...
/* ═══════════════════════════════════════════════════════════════
   ANIMATED COUNTER
═══════════════════════════════════════════════════════════════ */
const AnimCounter = memo(({ to, duration = 1.4 }) => {
...
═══════════════════════════════════════════════════════════════ */
const Ticker = memo(() => {
  const items = [...FUN_FACTS, ...FUN_FACTS, ...FUN_FACTS];
  return (
    <div style={{
      overflow: "hidden",
      borderTop: "1px solid rgba(232,98,42,0.1)",
      borderBottom: "1px solid rgba(232,98,42,0.1)",
...
/* ═══════════════════════════════════════════════════════════════
   CINEMATIC ANIMATION WRAPPERS
═══════════════════════════════════════════════════════════════ */
const CinematicParagraph = memo(({ children, style, delay = 0 }) => {
  const ref = useRef(null);
...
CinematicParagraph.displayName = "CinematicParagraph";

const CinematicSection = memo(({ children, style, delay = 0 }) => {
  const ref = useRef(null);
...
FloatLine.displayName = "FloatLine";

/* ═══════════════════════════════════════════════════════════════
   ACHIEVEMENT CARD — 3D tilt + glare
═══════════════════════════════════════════════════════════════ */
const AchievementCard = memo(({ item, index, isMobile }) => {
  ...
  return (
    ...
    >
      ...
      <div style={{
        ...
        borderRadius: 18,
        background: hov
          ? "linear-gradient(145deg,rgba(14,11,8,0.98),rgba(10,8,6,0.98))"
          : "linear-gradient(145deg,rgba(9,8,6,0.9),rgba(7,6,5,0.85))",
        ...
        <div style={{ position: "relative" }}>
          ...
          <h4 style={{
            ...
            fontSize: isMobile ? "clamp(1.5rem,7vw,2rem)" : "clamp(1.8rem,3vw,2.6rem)",
            fontWeight: 400, letterSpacing: "0.04em", color: T.text, margin: "0 0 0.15rem 0", lineHeight: 1,
          }}>{item.count}</h4>
...
/* ═══════════════════════════════════════════════════════════════
   TIMELINE — updated to 2026 + enhanced design
═══════════════════════════════════════════════════════════════ */
const Timeline = memo(({ isMobile }) => {
  const ref    = useRef(null);
  ...
  return (
    <CinematicSection style={{ display: "flex", flexDirection: "column", gap: 0 }}>
      <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", marginBottom: "1.4rem" }}>
        ...
        <span style={{ fontFamily: "'Space Mono',monospace", fontSize: "0.42rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.3em", color: T.faint }}>
          Journey
        </span>
      </div>
      ...
      <div ref={ref} style={{ position: "relative", paddingLeft: "1.8rem" }}>
        ...
        {TIMELINE.map((item, i) => (
          <motion.div
            ...
            style={{ position: "relative", marginBottom: i < TIMELINE.length - 1 ? "1.6rem" : 0, paddingLeft: "0.8rem" }}
          >
            {/* Dot */}
            <div style={{
              position: "absolute", left: "-1.3rem", top: 3,
              width: item.active ? 12 : 10, height: item.active ? 12 : 10,
              borderRadius: "50%",
              background: item.active ? item.color : "rgba(232,98,42,0.2)",
              border: `1px solid ${item.color || T.orange}`,
              display: "flex", alignItems: "center", justifyContent: "center",
              zIndex: 1,
              boxShadow: item.active ? `0 0 10px ${item.color}66` : "none",
            }}>
              {item.active && (
                <motion.div
            ...
            <div style={{ display: "flex", alignItems: "baseline", gap: "0.5rem", marginBottom: 3, flexWrap: "wrap" }}>
              <span style={{
                fontFamily: "'Bebas Neue',sans-serif", fontSize: "0.95rem",
                color: item.color || T.orange, letterSpacing: "0.06em",
              }}>{item.year}</span>
              <span style={{
                fontFamily: "'Space Mono',monospace", fontSize: "0.5rem", fontWeight: 700,
                color: T.text, letterSpacing: "0.1em", textTransform: "uppercase",
              }}>{item.title}</span>
              <span style={{ fontSize: 11, marginLeft: "auto" }}>{item.icon}</span>
            </div>
            <p style={{ fontFamily: "'Space Mono',monospace", fontSize: "0.56rem", color: T.muted, margin: 0, lineHeight: 1.65 }}>
              {item.detail}
            </p>
          </motion.div>
        ))}
...
/* ═══════════════════════════════════════════════════════════════
   LIVE PROJECTS STRIP — premium feature
═══════════════════════════════════════════════════════════════ */
const LiveProjectsStrip = memo(({ isMobile }) => {
        ...
        {LIVE_PROJECTS.map((p, i) => (
          <motion.a
            ...
            style={{
              ...
              background: hovered === i ? "rgba(232,98,42,0.05)" : "rgba(255,255,255,0.02)",
              border: `1px solid ${hovered === i ? T.borderH : T.borderB}`,
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: "flex", alignItems: "baseline", gap: "0.4rem", flexWrap: "wrap" }}>
                <span style={{ fontFamily: "'Space Mono',monospace", fontSize: "0.54rem", fontWeight: 700, color: hovered === i ? T.orangeG : T.text }}>
                  {p.name}
                </span>
                <span style={{ fontFamily: "'Space Mono',monospace", fontSize: "0.38rem", color: T.faint }}>{p.year}</span>
              </div>
              <span style={{ fontFamily: "'Space Mono',monospace", fontSize: "0.46rem", color: T.muted, lineHeight: 1.5 }}>{p.desc}</span>
            </div>
            <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 4, flexShrink: 0 }}>
              <span style={{ fontFamily: "'Space Mono',monospace", fontSize: "0.38rem", color: T.green, whiteSpace: "nowrap" }}>{p.impact}</span>
              <span style={{ color: hovered === i ? T.orange : T.faint, transition: "color 0.2s" }}>
                {ICONS.externalLink(9, "currentColor")}
              </span>
            </div>
          </motion.a>
        ))}
...
LiveProjectsStrip.displayName = "LiveProjectsStrip";

/* ═══════════════════════════════════════════════════════════════
   STAT STRIP
═══════════════════════════════════════════════════════════════ */
const StatStrip = memo(({ isMobile }) => {
  const stats = [
...
/* ═══════════════════════════════════════════════════════════════
   SKILL TAG
═══════════════════════════════════════════════════════════════ */
const SkillTag = memo(({ skill, index, color }) => {
  const [hov, setHov] = useState(false);
  ...
  return (
    <motion.span
      ...
      style={{
        ...
        background: hov ? accentAlpha : "rgba(255,255,255,0.03)",
        border: `1px solid ${hov ? accentBorder : T.borderB}`,
        color: hov ? accent : T.muted,
...
/* ═══════════════════════════════════════════════════════════════
   SKILLS PANEL — enhanced with color-coded tabs + copy email
═══════════════════════════════════════════════════════════════ */
const SkillsPanel = memo(({ isMobile }) => {
  const ref = useRef(null);
  const mx  = useMotionValue(0); const my  = useMotionValue(0);
  ...
  const [skillSearch, setSearch]  = useState("");
  ...
  const filteredSkills = skillSearch
    ...
    : cat.skills;
  const displayColor = skillSearch ? T.orange : cat.color;
    ...
    >
      ...
      <CinematicSection style={{
        ...
        <div style={{ position: "relative" }}>
          ...
          <div style={{ marginBottom: "0.9rem", position: "relative" }}>
            <input
              ...
              style={{
                ...
                border: `1px solid ${skillSearch ? T.border : T.borderB}`,
                color: T.text, fontFamily: "'Space Mono',monospace", fontSize: "0.52rem",
                outline: "none", transition: "border-color 0.2s",
              }}
            />
            <span style={{ position: "absolute", left: "0.6rem", top: "50%", transform: "translateY(-50%)", color: T.muted, fontSize: 10 }}>⌕</span>
            {skillSearch && (
              <button onClick={() => setSearch("")} style={{
                position: "absolute", right: "0.6rem", top: "50%", transform: "translateY(-50%)",
                background: "none", border: "none", color: T.muted, cursor: "pointer", fontSize: 12, lineHeight: 1,
              }}>×</button>
            )}
          </div>
          ...
          {!skillSearch && (
            <div style={{ display: "flex", gap: 4, marginBottom: "1.2rem", background: "rgba(255,255,255,0.025)", borderRadius: 8, padding: 4, border: "1px solid rgba(255,255,255,0.05)" }}>
              {SKILL_CATEGORIES.map((c, i) => (
                <button key={i} onClick={() => setTab(i)} style={{
                  ...
                  background: activeTab === i ? `${c.color}22` : "transparent",
                  color: activeTab === i ? c.color : "rgba(242,238,248,0.28)",
                  borderBottom: activeTab === i ? `1px solid ${c.color}55` : "1px solid transparent",
          ...
          <AnimatePresence mode="wait">
            <motion.div key={skillSearch ? "search" : activeTab}
            ...
            >
              {!skillSearch && (
                <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "0.8rem" }}>
                  <div style={{
                    ...
                    {ICONS[cat.icon]?.(10, cat.color)}
                  </div>
                  <span style={{ fontFamily: "'Space Mono',monospace", fontSize: "0.44rem", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.2em", color: cat.color }}>
                    {cat.title}
                  </span>
                  {activeTab === 0 && (
              ...
              {skillSearch && filteredSkills.length === 0 && (
                <p style={{ fontFamily: "'Space Mono',monospace", fontSize: "0.5rem", color: T.muted, textAlign: "center", padding: "1rem 0" }}>
                  No skills matching "{skillSearch}"
                </p>
              )}
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.35rem" }}>
                {filteredSkills.map((skill, si) => (
                  <SkillTag key={skill} skill={skill} index={si} color={displayColor} />
                ))}
              </div>
            </motion.div>
          </AnimatePresence>
          ...
          <motion.div
            ...
            style={{
              marginTop: "1.4rem", padding: isMobile ? "0.85rem 1rem" : "1rem 1.15rem",
              background: "rgba(34,197,94,0.048)", border: "1px solid rgba(34,197,94,0.2)",
              borderRadius: 12, display: "flex", alignItems: "center", gap: "0.85rem",
            }}
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <p style={{ fontFamily: "'Space Mono',monospace", fontSize: "0.4rem", textTransform: "uppercase", letterSpacing: "0.22em", color: "rgba(34,197,94,0.72)", fontWeight: 700, marginBottom: 2, marginTop: 0 }}>
                Available Now
              </p>
          ...
          <motion.button
            ...
            style={{
              ...
              color: copied ? T.green : T.muted, transition: "all 0.25s",
            }}
          >
            <AnimatePresence mode="wait">
              ...
              >
                {copied ? ICONS.check(10, T.green) : ICONS.copy(10, "currentColor")}
          ...
          <Link to="/contact" style={{ textDecoration: "none" }}>
            <motion.div
              ...
              whileTap={{ scale: 0.97 }}
              style={{
                marginTop: "0.75rem", display: "flex", alignItems: "center", justifyContent: "center", gap: "0.55rem",
                padding: isMobile ? "0.82rem 1.4rem" : "0.9rem 1.6rem", borderRadius: 6,
                background: `linear-gradient(135deg,${T.orange},${T.orangeD} 65%,#b03a0e)`,
                color: "#fff", fontFamily: "'Space Mono',monospace",
                fontSize: isMobile ? "0.55rem" : "0.58rem", fontWeight: 700,
...
/* ═══════════════════════════════════════════════════════════════
   SECTION HEADER
═══════════════════════════════════════════════════════════════ */
const SectionHeader = memo(({ isMobile }) => {
    ...
    <motion.header ref={headerRef} style={{ opacity, marginBottom: isMobile ? "3rem" : "4.5rem", position: "relative" }}>
      ...
      >
        ...
        <div style={{
          ...
          <span style={{ fontFamily: "'Space Mono',monospace", fontSize: "0.46rem", fontWeight: 700, letterSpacing: "0.26em", textTransform: "uppercase", color: "rgba(240,132,90,0.95)", whiteSpace: "nowrap" }}>
            About Me
          </span>
        </div>
        ...
        <motion.div style={{ y: isMobile ? 0 : y }}>
          <motion.h2
            ...
            style={{
              ...
              fontWeight: 400, lineHeight: 0.88, letterSpacing: "0.02em", color: T.text, margin: 0,
            }}
          >
            Crafting Worlds
            <br />
    ...
    >
      ...
      <style>{`
        ...
        *,*::before,*::after { box-sizing: border-box; }
        ...
        input::placeholder { color: rgba(242,238,248,0.2); }
      `}</style>
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto", position: "relative", zIndex: 10 }}>
        ...
        <div style={{ marginTop: isMobile ? "3.5rem" : "5rem" }} className="ag">
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: isMobile ? "2rem" : "2.8rem" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.9rem" }}>
                ...
                <CinematicParagraph delay={0.25}
                  style={{
                    ...
                    fontWeight: 400, letterSpacing: "0.07em", color: T.faint,
                    lineHeight: 1.65, borderLeft: `2px solid rgba(232,98,42,0.2)`,
            ...
            {/* Live Projects */}
            <LiveProjectsStrip isMobile={isMobile} />
          ...
          {/* ── RIGHT COLUMN: sticky skills panel ── */}
