...
/* ═══════════════════════════════════════════════════════════════
   DATA — 3D/WebGL skills added, WordPress removed
═══════════════════════════════════════════════════════════════ */
...
// Achievements — replaced "100% Responsive" and "∞ Learning" with real differentiators
const ACHIEVEMENTS = [
...
/* ═══════════════════════════════════════════════════════════════
   ...
   Lightweight — uses lower geometry counts than Hero.
   Floating icosahedra + point cloud + halo rings.
   Responds to scroll for parallax.
═══════════════════════════════════════════════════════════════ */
function AboutThreeScene({ isMobile }) {
  ...
  useEffect(() => {
    ...
    // Lights — subtle, no shadows (background scene)
    ...
    // Spread across the section — more sparse than Hero
    ...
    // ── Central accent torus knot (right side, behind content) ──
    ...
    // ── Horizontal scan lines (2 thin rings sweeping) ──
    ...
    const animate = () => {
      ...
      shapes.forEach(({ mesh, rotSpeed, phase, floatSpeed, floatAmp, axis, originY }) => {
        ...
        fadeTo(mesh.material, mesh.material.opacity < mesh.material._target
          ? mesh.material.opacity + 0.02
          : mesh.material.opacity, 0.02);
      });
      // Set opacity targets once
      shapes.forEach(s => { if (!s._init) { s._init=true; s.mesh.material._target = s.mesh.material.opacity; } });

      // Points
      fadeTo(ptMat, 0.55 * eased, 0.025);
      points.rotation.y = t * 0.008;

      // Scan lines
      scan1.position.y = ((t * 0.4) % 8) - 4;
      scan2.position.y = ((-t * 0.28 + 3) % 8) - 4;
      fadeTo(scanMat1, 0.08 * eased, 0.02);
      fadeTo(scanMat2, 0.06 * eased, 0.02);

      // Lights drift
      l1.position.x = Math.sin(t * 0.22) * 7;
      l1.position.y = Math.cos(t * 0.16) * 4 + 2;
      l2.position.x = Math.cos(t * 0.19) * 6;
      l2.position.z = Math.sin(t * 0.14) * 4;
      ...
      <CinematicSection style={{
        ...
        <div style={{ position:"relative" }}>
            ...
            {SKILL_CATEGORIES.map((cat, idx) => (
              <motion.div key={idx}
                ...
                <div style={{ display:"flex", alignItems:"center", gap:"0.5rem" }}>
                  <div style={{ width:20, height:20, borderRadius:5, background:idx===0?"rgba(232,98,42,0.12)":"rgba(232,98,42,0.08)", border:`1px solid ${idx===0?"rgba(232,98,42,0.25)":"rgba(232,98,42,0.15)"}`, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                    {ICONS[cat.icon]?.(10, idx===0 ? T.orange : "rgba(232,98,42,0.75)")}
                  </div>
                  <span style={{ fontFamily:"'Space Mono',monospace", fontSize:"0.44rem", fontWeight:700, textTransform:"uppercase", letterSpacing:"0.2em", color: idx===0 ? "rgba(240,132,90,0.7)" : "rgba(255,255,255,0.48)" }}>
                    {cat.title}
                  </span>
                  {/* 3D label */}
                  {idx === 0 && (
                ...
                <div style={{ display:"flex", flexWrap:"wrap", gap:"0.35rem" }}>
                  {cat.skills.map((skill, si) => (
                    <SkillTag key={skill} skill={skill} index={si} is3D={idx===0}/>
                  ))}
                </div>
              </motion.div>
            ))}
          ...
          <motion.div
            ...
            style={{ marginTop: isMobile ? "1.4rem" : "2rem", padding: isMobile ? "0.85rem 1rem" : "1rem 1.15rem", background:"rgba(34,197,94,0.048)", border:"1px solid rgba(34,197,94,0.2)", borderRadius:12, display:"flex", alignItems:"center", gap:"0.85rem" }}>
          ...
          <Link to="/contact" style={{ textDecoration:"none" }}>
            <motion.div
              ...
              whileTap={{ scale:0.97 }}
              style={{ marginTop:"1rem", display:"flex", alignItems:"center", justifyContent:"center", gap:"0.55rem", padding: isMobile?"0.82rem 1.4rem":"0.9rem 1.6rem", borderRadius:6, background:`linear-gradient(135deg,${T.orange},${T.orangeD} 65%,#b03a0e)`, color:"#fff", fontFamily:"'Space Mono',monospace", fontSize: isMobile?"0.55rem":"0.58rem", fontWeight:700, letterSpacing:"0.22em", textTransform:"uppercase", boxShadow:`0 8px 28px rgba(232,98,42,0.28),inset 0 1px 0 rgba(255,255,255,0.18)`, cursor:"pointer", position:"relative", overflow:"hidden", width:"100%" }}>
...
export default function About() {
  ...
  return (
    <section id="about" ref={sectionRef}
      ...
      {/* Diagonal accent */}
      {!isMobile && (
        <div style={{ position:"absolute", top:0, left:"38%", width:1, height:"100%", background:"linear-gradient(to bottom,transparent,rgba(232,98,42,0.09) 45%,transparent)", transform:"rotate(-8deg)", pointerEvents:"none", zIndex:0 }}/>
      )}
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:"1.1rem" }}>
              ...
              <div style={{ display:"flex", flexDirection:"column", gap:"0.9rem" }}>
                <CinematicParagraph delay={0.07}
                  ...
                  {" "}— interactive product visualisers, WebGL environments, shader-driven effects — and back them with full-stack architecture that actually ships. My differentiator is combining technical 3D depth with the product instincts to know when it serves the user.
                </CinematicParagraph>
                ...
                <CinematicParagraph delay={0.13}
                  ...
                  , manage data flow and auth, and integrate third-party services. I care about{" "}
                  <span style={{ color:"rgba(240,132,90,0.85)" }}>performance, render budgets and maintainability</span>
                  {" "}— especially important when your frontend is doing real-time 3D work.
                </CinematicParagraph>

                <CinematicParagraph delay={0.19}
                  style={{ fontFamily:"'Space Mono',monospace", fontSize:monoSm, color:T.muted, lineHeight:1.9, margin:0 }}>
                  I collaborate with designers and product teams to turn concepts into{" "}
